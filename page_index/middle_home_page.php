...
<div id="homePage" class="subpage">
      ...
      <div id="tweets">
        ...
        <?php
          ...
          try {
            // $q = $db->prepare('SELECT * FROM tweets ORDER BY tweets.tweet_id DESC LIMIT 10');
            $q = $db->prepare('SELECT users.user_id, users.user_profile_name, users.user_image, tweets.tweet_id, tweets.tweet_user_fk, tweets.tweet_message, tweets.tweet_total_likes, tweets.tweet_total_comments, tweets.tweet_image FROM users JOIN tweets ON users.user_id = tweets.tweet_user_fk ORDER BY tweets.tweet_id DESC LIMIT 10');
            $q->execute();
            $ajTweets = $q->fetchAll();
            // echo json_encode($ajTweets);

            foreach($ajTweets as $ajTweet){
              // include(__DIR__.'/middle-tweet.php');
              // $user_image =  htmlspecialchars($ajTweet->user_image);

              $tweet_id =  htmlspecialchars($ajTweet[3]); //id
              $user_image =  htmlspecialchars($ajTweet[2]); //img
              $user_id =  htmlspecialchars($ajTweet[0]); //user id
              $user_name = htmlspecialchars($ajTweet[1]); //user name
              $tweet_message = htmlspecialchars($ajTweet[5]); //message
              $tweet_image = htmlspecialchars($ajTweet[8]); //tweet image
              $tweet_total_likes = htmlspecialchars($ajTweet[6]); //tweet total likes
              $tweet_total_comments = htmlspecialchars($ajTweet[7]); //tweet total likes


              $middle_post = file_get_contents(__DIR__.'/middle-home-tweet.html');
              $middle_post = str_replace('{{tweetId}}', $tweet_id, $middle_post);
              $middle_post = str_replace('{{userId}}', $user_id, $middle_post);
              $middle_post = str_replace('{{username}}', $user_name, $middle_post);
              $middle_post = str_replace('{{tweetmesage}}', $tweet_message, $middle_post);
              $middle_post = str_replace('{{tweetimage}}', $tweet_image, $middle_post);
              $middle_post = str_replace('{{userimage}}', $user_image, $middle_post);
              $middle_post = str_replace('{{tweettotallikes}}', $tweet_total_likes, $middle_post);
              $middle_post = str_replace('{{tweettotalcomments}}', $tweet_total_comments, $middle_post);
              echo $middle_post;
            };
